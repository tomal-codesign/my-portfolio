...
const SocialButton = ({ icon, text, iconColor, hoverColor, link, textHidenMobile = false }: props) => {
  ...
  return (
    <>
      <a href={link} className={`flex items-center gap-2 bg-[#fff]/70 py-2 pl-2 ${textHidenMobile ? '!pr-2' : ''} pr-5 rounded-full text-[var(--normal-text)] ${hoverColor} transition-colors duration-200 hover:text-white group`} target='_blank'>
